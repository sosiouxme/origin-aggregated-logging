...
CONF
	...
	sh scripts/generatePEMCert.sh "$admin_user"
...
echo "(Re-)Creating deployed objects"
# this is separate from the support-pre-template objects as we aren't automatically recreating the route and oauthclient
if [ "${KEEP_SUPPORT}" != true ]; then
	oc delete oauthclient,route --selector logging-infra=support
	oc process logging-support-template | oc create -f -
fi
